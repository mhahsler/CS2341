...
TEST_CASE("Simple Factorials", "[factorial]")
{
    ...
    // 1. The following test fails because we return an unsigned int. Another design issue!
    CHECK(factorial(100) == 9.332622e+157);

    // 2. The following test fails because we can't test negative numbers because the argument is unsigned! 
